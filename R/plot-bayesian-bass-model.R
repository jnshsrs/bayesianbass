...
plot.bayesian_bass <- function(x, se = F, ...) {
    ...
    p <- ggplot(pred, aes(x = t, y = predicted)) + geom_line() + geom_point(data = adoption_rates, aes(x = t, y = avg), inherit.aes = FALSE) + 
        scale_y_continuous("Proportional Adoption", labels = scales::percent) + scale_x_date("Year", date_breaks = "5 years", date_labels = "%Y") + 
        theme_minimal()
    
    if (se) {
        p <- p + geom_errorbar(data = adoption_rates, aes(ymin = Lower, ymax = Upper, x = t), width = 250, inherit.aes = FALSE)
    }
